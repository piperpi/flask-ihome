...
from ihome.utils.aliyun_oss import img_store
...
def get_areas_info():
    ...
    resp_dict = dict(errno=RET.OK, errmsg="OK", data=area_list)
    resp_json = json.dumps(resp_dict)
    try:
        redis_store.setex("areas_info", constants.AREA_INFO_REDIS_EXPIRES, resp_json)
    except Exception as e:
        current_app.logger.error(e)
...
def save_house_image():
    ...
    if '.' in image_file_name:
        image_file_format = image_file_name.rsplit('.')[-1]
    else:
    ...
    # 保存图片到阿里云中
    try:
        img_url = img_store(image_data,format=image_file_format)
    except Exception as e:
        current_app.logger.error(e)
        return jsonify(errno=RET.THIRDERR, errmsg="保存图片失败")

    # 保存图片信息到数据库中
    house_image = HouseImage(house_id=house_id, url=img_url)
    ...
    return jsonify(errno=RET.OK, errmsg="OK", data={"image_url": img_url})
